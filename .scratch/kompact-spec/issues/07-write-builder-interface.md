...
- The writer API mirrors the read side: `writeScalar(type: ScalarType, value: Long)` (two's-complement magnitude dispatched to `writeBits`/`writeBitsLong` by width band per ticket 04), `writeBool`, `writeString`/`writeBlob` (length-prefixed), `writeNested { w -> … }`, `writeRepeated(count, countWidth) { w -> … }`. Each typed write carries the field's compile-time-validated length-prefix width and value width — so a structurally-invalid stream is impossible to produce.
