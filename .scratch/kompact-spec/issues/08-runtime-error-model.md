...
- A checked accessor (`readScalar(raw, bitOffset, type: ScalarType): IntResult` and its siblings `readBool`, `readScalarAsLong`, `readFloat`, `readDouble`) bounds-checks first; on success it reads via `readBits`/`readBitsLong` and returns the corresponding result class's `success(value)` (zero-alloc); on a `BoundsError`/`BadLengthPrefix` it returns `failure(error)` (still a packed-Long, zero-alloc). It never throws.
...
- JVM Java interop: checked `readScalarOrThrow(raw, bitOffset, type: ScalarType): Int`-style wrappers are provided (Java callers see the result class); the common / public Kotlin API is the typed result value class.
