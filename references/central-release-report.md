...
- Point-of-risk publish confirmation: **Pending** — `centralPortalPublish` is IRREVERSIBLE. Deployment UUID `⟦REDACTED⟧` is `VALIDATED` (zero errors) and ready for publish.

## Release

- Source commit/tag: `feat/laguna` branch.
- Build tool and publisher/plugin version: Gradle 9.7.1, Kotlin 2.3.21, KMP plugin (version-aligned to Kotlin). No third-party publishing plugin — direct Portal Publisher API integration via `java.net.http.HttpClient`.
...
- PGP signing key: short key ID `⟦REDACTED⟧` (last 8 of `⟦REDACTED⟧...`); Gradle signing plugin rejects 16-char key IDs without `0x` prefix. Short ID form resolves this. The `.env` file retains the full 16-char ID; normalization to short form is done in the loader script, not in source.
...
- Signing-key: PGP private key in `.env` (`SIGNING_KEY`), key ID `⟦REDACTED⟧` (long) → normalized to `⟦REDACTED⟧` (short 8-char) for Gradle signing plugin. Passphrase via `SIGNING_PASSWORD` env var. PGP private key never committed to source.
...
- Unpublished/dropped deployments: Deployment `⟦REDACTED⟧...` is `VALIDATED` and kept (not dropped). Ready for publish or can be dropped via DELETE `/api/v1/publisher/deployment/<id>`.
...
- Signing key ID normalization: Gradle's `PgpKeyId.normaliseKeyId` rejects 16-char key IDs without `0x` prefix. Fixed by using the short 8-char key ID (`⟦REDACTED⟧`) in the loader script. This is the only workaround needed — the `.env` file is unchanged.

## What's left (authorization-gated)

These steps require **explicit user confirmation** before execution:

1. **PUBLISH** — Run `gradle :kompact:centralPortalPublish` (IRREVERSIBLE). User must confirm at point of risk. Deployment UUID `⟦REDACTED⟧` is `VALIDATED` (zero errors).
...
**Note on `SIGNING_KEY_ID`**: The `.env` file contains the 16-char long key ID (`⟦REDACTED⟧`). Gradle's signing plugin rejects 16-char IDs without `0x` prefix. The loader script in `centralPortalDeploy` normalization step shortens to the last 8 chars (`⟦REDACTED⟧`) before running Gradle. The `.env` file itself is NOT modified.
