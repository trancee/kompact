...
public actual value class IntResult(
    ...
    public actual fun getOrThrow(): Int =
        if (isSuccess) {
            (packed and RESULT_VALUE_MASK).toInt()
        } else {
            throwDecodeErrorFromSmallBits(packed)
        }

    public actual companion object {
        public actual fun success(value: Int): IntResult = IntResult(encodeSmallSuccess(value.toLong()))

        public actual fun failure(error: KompactDecodeError): IntResult = IntResult(encodeSmallFailure(error))
    }
}
...
public actual value class FloatResult(
    ...
    public actual fun getOrThrow(): Float =
        if (isSuccess) {
            Float.fromBits((packed and RESULT_VALUE_MASK).toInt())
        } else {
            throwDecodeErrorFromSmallBits(packed)
        }
    ...
    public actual companion object {
        ...
        public actual fun failure(error: KompactDecodeError): FloatResult = FloatResult(encodeSmallFailure(error))
    }
}

public actual value class BooleanResult(
    public actual val packed: Long,
) {
    public actual val isSuccess: Boolean get() = (packed and RESULT_OK_FLAG) != 0L
    public actual val isFailure: Boolean get() = (packed and RESULT_OK_FLAG) == 0L
    ...
    public actual fun getOrThrow(): Boolean =
        if (isSuccess) {
            (packed and RESULT_VALUE_MASK) != 0L
        } else {
            throwDecodeErrorFromSmallBits(packed)
        }
    ...
    public actual companion object {
        ...
        public actual fun failure(error: KompactDecodeError): BooleanResult = BooleanResult(encodeSmallFailure(error))
    }
}
...
public actual value class DoubleResult(
    ...
    public actual fun getOrThrow(): Double =
        if (isSuccess) {
            Double.fromBits(packed)
        } else {
            throwDecodeErrorFromDouble(packed)
        }

    public actual companion object {
        public actual fun success(value: Double): DoubleResult = DoubleResult(encodeDoubleSuccess(value))
